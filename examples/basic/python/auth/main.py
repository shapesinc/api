...
async def run():
    ...
    try:
        ...
        # Auth token is generated via this method.
        await authorise(code)
        #;Note: Auth token will be stored as `user.auth_token` for this specific user instance only.
        # You might want to store the auth token so that you don't have to keep authorising you every time the code is run.
